...
つぎに、選定したCSSモジュールごとに、対応の可否を一目で識別できるテスト用EPUBファイルを制作した。これは以下のリポジトリで公開しているので、お読みの方はぜひご自分でも試していただきたい<span class="notetext">なお、ライセンスは[CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/deed.ja)で、著作権を放棄している。詳細はリポジトリを参照。</span>

- [EPUBリーダーのCSS仕様適合性テスト](https://github.com/jagat-xpub/epub-css-test/tree/main)

最後に、テストベッドとなるEPUBリーダーを選定し、当研究会のメンバーごとに割り振ってテストを開始した。各担当者は前述テスト用EPUBファイルを自分が担当するEPUBリーダーにサイドロードし、テスト結果を以下のGoogleスプレッドシートに記録していったのである。<span class="notetext">「かなり安定しているが実装経験が限定的」「大まかな相互運用性がある」「CSS Snapshot 2023に載っていないが最新のブラウザで利用できる」に分類されるCSSモジュールの明細は、「EPUBリーダーのCSS仕様適合性テスト」を参照。</span>
...
ただし、実際に調査結果をまとめてみると、あまりにテスト項目が多すぎて収まりきらないことが判明した。やむなく本報告書ではEPUB 3.3で利用可能なモジュール（前述「CSSの公式的な定義に含まれるCSSモジュール」）に絞って結果を掲載することにした。その他のテスト結果を知りたい方は、申し訳ないが上記「EPUBリーダー表示チェック」を直接ご参照いただきたい。

<div class="no-indent" style="margin-top:24.5Q;">

**情報開示：**CSSモジュールの選定、及びテストファイルの制作は本稿の共著者の一人でもある村上真雄（Vivliostyle Foundation）がおこなった。村上はテスト対象となったEPUBリーダーの一つであるVivliostyle Viewerの開発者でもある。ただし、テストの制作およびその実施にあたって、中立性の確保、及び情報・プログラムコードの公開を徹底することで、信頼性の担保に努めた。この点、あらかじめ開示する。
...
テスト結果を報告する前に、まずレイアウトエンジンとは何か、今回のCSS表示テストとどのような関係があるのかを説明しよう。ここでいうレイアウトエンジンとは、文字や画像を特定のルールに基づき描画するプログラムのことだ。レンダリングエンジンともいう。たとえばインターネットブラウザの場合、前述ルールとはHTML、CSS、JavaScriptにあたる。同じくEPUBもこれらのルールに基づくことから、EPUBリーダーではインターネットブラウザのレイアウトエンジンが使われることも多い。

一般にレイアウトエンジンはそれ自体が巨大で複雑なプログラムであることから、開発はApple、Google、あるいはMozillaといった大規模なベンダーが担うことが多い。EPUBリーダーの開発に当たっては、これがネックとなる。
