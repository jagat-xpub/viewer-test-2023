...
独自エンジン系のEPUBリーダーは、EPUBではなく[電書協ガイド](http://ebpaj.jp/counsel/guide)のサポートを目指したのではないだろうか。

この電書協ガイドとは、電子書籍の出版社による業界団体、日本電子書籍出版社協会（現在は[デジタル出版者連盟](http://ebpaj.jp/)）によって、「加盟社のための一般書のEPUB 3を制作する際のガイド」として2012年に制定された規約だ。制定当時はEPUB 3.0に対応した[ver.1.1.2](http://ebpaj.jp/images/ebpaj_epub3guide_ver1.1.2.zip)だったが、その後EPUB 3.0.1（2014年）に対応した[ver.1.1.3 / 2015年1月1日版](http://www.idpf.org/epub/301/spec/epub-publications-20140626.html)にアップデートしている。

このver.1.1.3をダウンロードすると「20150101-CSS機能一覧.pdf」という文書があり、そこに利用可能なCSS機能がリストアップされている。そのほとんどを占めるプロパティは、CSS レベル2.1のサブセットだが、以下のようなCSS レベル3のプロパティもふくまれている（一部は当該仕様が当時は勧告前だったことから接頭辞を付けるよう指示）。
...
本テストでは、<a href="#2-1-2-テスト結果を掲載したcssモジュール">2-1-2</a>、及び<a href="#2-2-1-レイアウトエンジンと本テストとの関係">2-2-1</a>でも述べたように、まさかCSS レベル2.1をフルサポートしないEPUBリーダーがあるとは考えず、これをテスト項目から外していた。また、上記1〜3についても同じ理由でテスト項目に入れていたのは1だけだった。

したがって、電書協ガイドver.1.1.3が求める「CSS レベル2.1のサブセットと上記1〜3」のほとんどが本テストの対象外だ。その上での推測だが、独自エンジン系のリーダーがこの「20150101-CSS機能一覧.pdf」のサポートだけでよいと割り切った実装をしたとすれば、最新のCSSモジュールのサポートに消極的だった理由として説明がつく。

たとえば、<a href="#2-2-4-2writing-mode-プロパティ-vertical-rlに絞ったテスト結果">2-1-2</a>、及び<a href="#2-2-1-レイアウトエンジンと本テストとの関係">2-2-4-2</a>で述べたように、Kindle（Windows版）、kobo-other (Mac版／Windows版)、超縦書の4つはwriting-mode プロパティは接頭辞をつける必要がある。これはまさに電書協ガイドver.1.1.3通りなのだ。では、これら以外の独自エンジン系リーダーは、なぜ接頭辞をつけないよう修正したのかといえば、仕様が勧告になったのでその必要がなくなったと考えるのが自然だろう。むしろ勧告後も接頭辞付きのルールを変えない電書協ガイドの方が不自然と言える。

ただし、この「CSS レベル2.1のサブセットと上記1〜3」というのは、あくまでもEPUB制作者の便を図るための国内ルールであることを忘れてはいけないだろう。国内向けの制作ガイドとして、国際規格のサブセットを規定することはなんの問題もない。もっとも、それをリーダーが実装するとなると別次元の問題だ。万が一、電書協ガイドを実装したリーダーがあったとして、はたしてそれを「EPUB」リーダーと呼んでよいのか、いささか疑問が残るところではある。
