...
最後に、テストベッドとなるEPUBリーダーを選定し、当研究会のメンバーごとに割り振ってテストを開始した。各担当者は前述テスト用EPUBファイルを自分が担当するEPUBリーダーにサイドロードし、テスト結果を以下のGoogleスプレッドシートに記録していったのである<span class="notetext">前述「4つの規準」ごとに選定したCSSモジュール名も、これを参照してほしい。</span>
...
ただし、実際に調査結果をまとめてみると、あまりにテスト項目が多すぎて収まりきらないことが判明した。やむなく本報告書ではEPUB 3.3で利用可能なモジュール（前述「CSSの公式的な定義に含まれるCSSモジュール」）に絞って結果を掲載することにした。その他のテスト結果を知りたい方は、申し訳ないが上記「EPUBリーダー表示チェック」をご参照いただきたい。
...
テスト結果を報告する前に、まずレイアウトエンジンとは何か、今回おこなったCSS表示テストとどのような関係があるのかを説明しよう。ここでいうレイアウトエンジンとは、文字や画像を特定のルールに基づき描画するプログラムのことだ。レンダリングエンジンという。たとえばインターネットブラウザの場合、前述のルールはHTML、CSS、JavaScriptになる。同様にEPUBもこれらのルールに基づくことから、EPUBリーダーではインターネットブラウザのレイアウトエンジンが使われる場合も多い。

一般にレイアウトエンジンはそれ自体が巨大で複雑なプログラムであることから、開発はApple、Google、あるいはMozillaといった大規模なベンダーが担うことが多い。
