...
その背景には、WebKitが一定の条件で無料利用できるオープンソースソフトウェア（以下、OSS）であることから、これを使えば開発コストを抑えられ、かつ日本語書籍に必須とされる縦書きをはじめ、さまざまな機能を確保できたことが大きいと考えられる。

本稿では、Webの閲覧に現在使われている（つまりモダンな）ブラウザのレイアウトエンジンを利用したEPUBリーダーを、一括して**「モダンブラウザ系」**と呼ぶことにする。
...
ただし、これだけで簡単に分別できるほど現実は甘くない。レイアウトエンジンのバージョン、もっといえばアップデートの頻度が関わってくるからだ。
...
### 2-2-2 メジャーなEPUBリーダーでサポートされているCSSモジュール
...
### 2-2-3 おもにモダンブラウザ系でサポートされているCSSモジュール
