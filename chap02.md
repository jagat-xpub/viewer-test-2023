...
この項で報告するのは、6つのCSSモジュールにわたる、27項目のテスト結果である。ここで言う「メジャーなEPUBリーダー」の定義については前項でも説明したが、要は「ほとんどの」ということだ。つまり、このクループは「ほとんどのEPUBリーダーでサポートされるCSSモジュール」である。ではテスト結果をまとめた表をご覧いただきたい。
