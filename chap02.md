...
［情報開示］テスト対象のCSSモジュールの選定、及びテストファイルの制作は本稿の共著者の一人でもある村上真雄（Vivliostyle Foundation）がおこなった。村上はテスト対象となったEPUBリーダーの一つであるVivliostyle Viewerの開発者でもある。ただし、テストの制作、およびその実施にあたって、中立性の確保と情報・プログラムコードの公開を徹底することで、信頼性の担保に努めた。この点、あらかじめ開示する。
