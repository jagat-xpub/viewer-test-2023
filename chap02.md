...
テスト結果を報告する前に、まずレイアウトエンジンとは何か、これがCSS表示テストとどのような関係があるのかを説明しよう。

ここでいうレイアウトエンジンとは、文字や画像を特定のルールに基づき描画するプログラムのことだ。レンダリングエンジンともいう。たとえばインターネットブラウザの場合、前述ルールとはHTML、CSS、JavaScriptにあたる。同じくEPUBもこれらのルールに基づくことから、EPUBリーダーでもインターネットブラウザのレイアウトエンジンが使われることも多い。

たとえば、Appleによるオープンソースである[WebKit](https://webkit.org/)を使った「楽天Koboアプリ for iPhone」「楽天Koboアプリ for Android」「ブック」「MURASAKI」「BOOK☆WALKER」がそうである。一般にレイアウトエンジンはそれ自体が巨大で複雑なプログラムであることから、オープンソースを利用することで開発コストを下げることが狙いと考えられる<span class="notetext">ただし、AppleのiOSに関してはサードパーティーに対して自社製WebKit以外の利用を許していないことから、他の選択肢はない</span>。
